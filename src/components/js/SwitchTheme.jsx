...
const changeTheme = (pre, curr) => {
  ...
  document.getElementById('switcher_theme').classList.remove(pre)
  document.documentElement.classList.add(curr)
  document.getElementById('switcher_theme').classList.add(curr)
}
const changeTypo = (pre, curr) => {
  document.documentElement.classList.remove(pre)
  document.getElementById('switcher_typo').classList.remove(pre)
  document.documentElement.classList.add(curr)
  document.getElementById('switcher_typo').classList.add(curr)
}
  ...
  return (
    <>
      <div className="switcher disabled theme" onClick={switchTheme}>
        <div id="switcher_theme" className="theme1"></div>
      </div>
      <div className="switcher disabled typo" onClick={switchTypo}>
        <div id="switcher_typo" className="typo1"></div>
      </div>
    </>
  )
}
